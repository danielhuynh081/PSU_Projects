...
// This program demonstrates Unix signal handling, process creation, and child process management using alarms, forks, and signal handlers.
#include <stdio.h>
#include <signal.h>
#include <stdlib.h>
#include <unistd.h>
...
void sigint_handler(int sig){
	printf("Signal for %d caught", sig);
...
void sigchild_handler(int sig){
	...
	while (( cpid =waitpid(-1, &status, WNOHANG)) > 0){
		printf("\n{arent signal handler: Found child exit %d: pid: %d exit value: %d\n", sig, cpid, WEXITSTATUS(status));
	      num_fork--;
	}
	if(num_fork == 0){
