...
// gcc -g -Wall -DNOISY_DEBUG -o vector vectord.c
// Daniel Huynh, CS333, Jesse Chaney
// This program reads, sorts, and outputs a collection of 2D points using dynamic memory allocation and command-line file I/O.
...
int main(int argc, char* argv[]){
		...
		while((opt = getopt(argc, argv, OPTIONS)) != -1) {
			switch(opt){
				case 'i':
					...
					if(!ifile){
						perror("open of input file fialed");
						exit(EXIT_FAILURE);
					}
					break;
				case 'o':
					...
					if(!ofile){
						perror("open of output file fialed");
						exit(EXIT_FAILURE);
					}
					break;
				case 'v':
					...
					break;
				case 'h':
					printf("sure, a lot of helpful infomration goes here\n");
					exit(EXIT_SUCCESS);
					break;
				default:
					break;
	...
	NOISY_DEBUG_PRINT;
	if(is_verbose){
		fprintf(stderr, "sorting by acending value\n");
	}
...
point_t * read_data(FILE * ifile, int * num_elements)
{
	...
	while(fgets(buffer, BUF_SIZE, ifile) != NULL){
		...
		while(token){
			...
			if(result != 3){
				fprintf(stderr, "badparse of input data %d %s\n", result, token);
				exit(EXIT_FAILURE);
			}
