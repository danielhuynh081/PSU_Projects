...
// This program implements a simplified version of the Unix head command for compressed files using processes, pipes, and exec().
	...
	}
	{
		...
		switch(pid){
			...
			default:
				{
					char * lhp_argv[] = {
						"zcat", file_name, (char *) NULL};
					if(dup2(pipes[STDOUT_FILENO], STDOUT_FILENO) < 0){
						perror("zhead 2 failed dup 2");
						return(EXIT_FAILURE);
					}
					close(pipes[STDIN_FILENO]);
					close(pipes[STDOUT_FILENO]);
